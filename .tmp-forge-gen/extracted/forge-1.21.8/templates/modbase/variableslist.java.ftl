...
@Mod.EventBusSubscriber(bus = Mod.EventBusSubscriber.Bus.MOD) public class ${JavaModName}Variables {
	...
	@Mod.EventBusSubscriber public static class EventBusVariableHandlers {
		...
		@SubscribeEvent public static void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
			if (!event.getEntity().level().isClientSide()) {
				...
				if(mapdata != null)
					${JavaModName}.PACKET_HANDLER.send(new SavedDataSyncMessage(0, mapdata), PacketDistributor.PLAYER.with((ServerPlayer) event.getEntity()));
				if(worlddata != null)
					${JavaModName}.PACKET_HANDLER.send(new SavedDataSyncMessage(1, worlddata), PacketDistributor.PLAYER.with((ServerPlayer) event.getEntity()));
			}
		}
	...
	public static class WorldVariables extends SavedData {
		...
		public void syncData(LevelAccessor world) {
			...
			if (world instanceof Level level && !level.isClientSide())
				${JavaModName}.PACKET_HANDLER.send(new SavedDataSyncMessage(1, this), PacketDistributor.DIMENSION.with(level.dimension()));
		}
	...
	public static class MapVariables extends SavedData {
		...
		public void syncData(LevelAccessor world) {
			...
			if (world instanceof Level && !world.isClientSide())
				${JavaModName}.PACKET_HANDLER.send(new SavedDataSyncMessage(0, this), PacketDistributor.ALL.noArg());
		}
	...
	public static class SavedDataSyncMessage {
		...
		public static void handler(SavedDataSyncMessage message, CustomPayloadEvent.Context context) {
			context.enqueueWork(() -> {
				if (context.isClientSide() && message.data != null) {
					if (message.type == 0)
	...
	public static class PlayerVariables {
		...
		public void syncPlayerVariables(Entity entity) {
			...
			${JavaModName}.PACKET_HANDLER.send(new PlayerVariablesSyncMessage(this), PacketDistributor.PLAYER.with(serverPlayer));
		}
	...
	public static class PlayerVariablesSyncMessage {
		...
		public static void handler(PlayerVariablesSyncMessage message, CustomPayloadEvent.Context context) {
			context.enqueueWork(() -> {
