...
@Mod.EventBusSubscriber(value = Dist.CLIENT) public class ${JavaModName}KeyMappings {
					...
					@Override public void setDown(boolean isDown) {
						...
						if (isDownOld != isDown && isDown) {
							<#if hasProcedure(keybind.onKeyPressed)>
								${JavaModName}.PACKET_HANDLER.send(new ${keybind.getModElement().getName()}Message(0, 0), PacketDistributor.SERVER.noArg());
						...
						<#if hasProcedure(keybind.onKeyReleased)>
							...
							${JavaModName}.PACKET_HANDLER.send(new ${keybind.getModElement().getName()}Message(1, dt), PacketDistributor.SERVER.noArg());
	...
	@Mod.EventBusSubscriber(value = Dist.CLIENT) public static class KeyEventListener {
