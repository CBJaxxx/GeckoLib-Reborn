	...
	public interface MenuAccessor {
		...
		default void sendMenuStateUpdate(Player player, int elementType, String name, Object elementState, boolean needClientUpdate) {
			...
			if (player instanceof ServerPlayer serverPlayer) {
				${JavaModName}.PACKET_HANDLER.send(new MenuStateUpdateMessage(elementType, name, elementState), PacketDistributor.PLAYER.with(serverPlayer));
			} else if (player.level().isClientSide) {
				if (Minecraft.getInstance().screen instanceof ${JavaModName}Screens.ScreenAccessor accessor && needClientUpdate)
					accessor.updateMenuState(elementType, name, elementState);
				${JavaModName}.PACKET_HANDLER.send(new MenuStateUpdateMessage(elementType, name, elementState), PacketDistributor.SERVER.noArg());
			}
		}
