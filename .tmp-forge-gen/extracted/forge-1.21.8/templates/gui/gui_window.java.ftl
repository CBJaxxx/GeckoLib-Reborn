				...
				${component.min}, ${component.max}, ${component.value}, ${component.step}, 0, true) {
					@Override protected void applyValue() {
						...
						<#if hasProcedure(component.whenSliderMoves)>
							${JavaModName}.PACKET_HANDLER.send(new ${name}SliderMessage(${slid}, x, y, z, this.getValue()), PacketDistributor.SERVER.noArg());
...
e -> {
	<#if hasProcedure(component.onClick)>
		...
		int y = ${name}Screen.this.y;
		if (<@procedureOBJToConditionCode component.displayCondition/>) {
			${JavaModName}.PACKET_HANDLER.send(new ${name}ButtonMessage(${btid}, x, y, z), PacketDistributor.SERVER.noArg());
