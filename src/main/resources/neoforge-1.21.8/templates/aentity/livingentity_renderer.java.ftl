...
public class ${name}Renderer extends GeoEntityRenderer<${name}Entity, ${name}RenderState> {
  public ${name}Renderer(EntityRendererProvider.Context renderManager) {
   ...
   public RenderType getRenderType(${name}Entity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
	...
	@Override
	public void preRender(PoseStack poseStack, ${name}Entity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int color) {
			<#if data.visualScale??>
