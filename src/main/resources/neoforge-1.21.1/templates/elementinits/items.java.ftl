...
<#if itemsWithInventory?size != 0 || animatedItemsWithInventory?size != 0 || buckets?size != 0>
@EventBusSubscriber
</#if>
public class ${JavaModName}Items {
	...
	<@javacompress>
	<#list items as item>
		<#if item.getModElement().getTypeString() == "armor" || item.getModElement().getTypeString() == "animatedarmor">
	...
	<#if has_chunks>public static void register${sub_items?index}()<#else>static</#if> {
		<#list sub_items as item>
			...
			<#elseif item.getModElement().getTypeString() == "animatedarmor">
				<#if item.enableHelmet>
				${item.getModElement().getRegistryNameUpper()}_HELMET =
					REGISTRY.register("${item.getModElement().getRegistryName()}_helmet", () -> new ${item.getModElement().getName()}Item(ArmorItem.Type.HELMET, new Item.Properties()<#if item.helmetImmuneToFire>.fireResistant()</#if>.durability(ArmorItem.Type.HELMET.getDurability(${item.maxDamage}))));
				</#if>
				<#if item.enableBody>
				${item.getModElement().getRegistryNameUpper()}_CHESTPLATE =
