...
public final class RegistryObject<T> implements Supplier<T> {
    ...
    public static <T, U extends T> RegistryObject<U> create(final ResourceLocation name, final ResourceKey<? extends Registry<T>> registryKey, String modid) {
        return new RegistryObject<>(name, registryKey.m_135782_(), modid, false);
    }
    ...
    public static <T, U extends T> RegistryObject<U> createOptional(final ResourceLocation name, final ResourceKey<? extends Registry<T>> registryKey, String modid) {
        return new RegistryObject<>(name, registryKey.m_135782_(), modid, true);
    }
    ...
    private RegistryObject(ResourceLocation name, IForgeRegistry<?> registry) {
        ...
        this.name = name;
        this.key = (ResourceKey<T>) ResourceKey.m_135785_(registry.getRegistryKey(), name);
        this.optionalRegistry = false;
    ...
    private RegistryObject(final ResourceLocation name, final ResourceLocation registryName, final String modid, boolean optionalRegistry) {
        this.name = name;
        this.key = ResourceKey.m_135785_(ResourceKey.m_135788_(registryName), name);
    ...
    RegistryObject(final ResourceLocation name, final ResourceKey<? extends Registry<? super T>> registry, final String modid) {
        ...
        this.key = ResourceKey.m_135785_(typed, name);
    ...
    void updateReference(IForgeRegistry<? extends T> registry) {
        ...
        if (registry.containsKey(this.name)) {
            ...
            this.value = null;
            this.holder = () -> wrapper.m_203505_().m_255043_(this.key);
        } else {
            this.value = null;
            this.holder = null;
        }
    ...
    void updateReference(Registry<? extends T> registry) {
        ...
        if (registry.m_7804_(this.name)) {
            this.value = registry.m_122327_(this.name);
            this.holder = () -> ((Registry<T>)registry).m_205904_(this.key).orElse(null);
        } else if (mapped != null && !mapped.isIntrusive()) {
            this.value = null;
            this.holder = () -> mapped.m_203505_().m_255043_(this.key);
        } else {
            this.value = null;
            this.holder = null;
        }
    ...
    void updateReference(ResourceLocation registryName) {
        ...
        Registry<? extends T> vanillaRegistry = (Registry<? extends T>) BuiltInRegistries.f_257047_.m_122327_(registryName);
    ...
    private static boolean registryExists(ResourceLocation registryName) {
        return RegistryManager.ACTIVE.getRegistry(registryName) != null
                || BuiltInRegistries.f_257047_.m_7804_(registryName);
    }
