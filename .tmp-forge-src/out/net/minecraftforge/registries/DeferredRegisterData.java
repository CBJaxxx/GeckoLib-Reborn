...
public class DeferredRegisterData<T> implements RegistryBootstrap<T> {
     ...
     */
    @Override
    public void m_254966_(BootstrapContext<T> context) {
        beenBuilt = true;

        var lookup = context.m_255434_(this.registryKey);
        ...
        for (var e : entries.entrySet()) {
            ...
            if (e.getValue() != null) {
                ...
                context.m_255042_(ro.getKey(), value, this.lifecycle);
            } else
                value = lookup.m_205904_(ro.getKey()).map(Holder::get).orElse(null);

            ro.updateReference(value, () -> lookup.m_255043_(ro.getKey()));
        }
    }
}
