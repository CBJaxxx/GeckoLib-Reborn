...
public record ItemExistsCondition(ResourceLocation item) implements ICondition {
    public static final MapCodec<ItemExistsCondition> CODEC = RecordCodecBuilder.mapCodec(b -> b.group(
        ResourceLocation.f_135803_.fieldOf("item").forGetter(ItemExistsCondition::item)
    ...
    public boolean test(IContext context, DynamicOps<?> ops) {
        ...
        if (ops instanceof RegistryOps<?> reg) {
            Optional<HolderGetter<Item>> items = reg.m_255006_(Registries.f_256913_);
            if (items.isPresent()) {
                var key = ResourceKey.m_135785_(Registries.f_256913_, this.item);
                return items.get().m_205904_(key).isPresent();
            }
        }
