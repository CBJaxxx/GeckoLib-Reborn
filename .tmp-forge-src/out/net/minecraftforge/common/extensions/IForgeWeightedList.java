...
public interface IForgeWeightedList<E> {
    ...
    default WeightedList<E> removeIf(Predicate<E> predicate) {
        var list = new ArrayList<>(this.self().m_387721_());
        return list.removeIf(w -> predicate.test(w.f_380007_()))
            ? WeightedList.<E>m_384576_().addAll(list).m_384449_()
            : this.self();
    }
    ...
    interface Builder<E> {
        ...
        default WeightedList.Builder<E> addAll(Iterable<Weighted<E>> iterable) {
            for (var w : iterable) this.self().m_385754_(w.f_380007_(), w.f_380172_());
            return this.self();
        }
        ...
        default WeightedList.Builder<E> removeIf(Predicate<E> predicate) {
            var list = new ArrayList<>(this.self().m_384449_().m_387721_());
            return list.removeIf(w -> predicate.test(w.f_380007_()))
                ? WeightedList.<E>m_384576_().addAll(list)
                : this.self();
        }
    }
}
