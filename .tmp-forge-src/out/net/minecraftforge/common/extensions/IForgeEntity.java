...
public interface IForgeEntity extends ICapabilitySerializable<CompoundTag> {
    ...
    default void deserializeNBT(HolderLookup.Provider registryAccess, CompoundTag nbt) {
        try (var problems = new ProblemReporter.ScopedCollector(self().m_403373_(), Entity.f_399955_)) {
            self().m_20258_(TagValueInput.m_401579_(problems, registryAccess, nbt));
        }
    }
    ...
    default CompoundTag serializeNBT(HolderLookup.Provider registryAccess) {
        try (var problems = new ProblemReporter.ScopedCollector(self().m_403373_(), Entity.f_399955_)) {
            var output = TagValueOutput.m_402504_(problems, registryAccess);
            var id = self().m_20078_();
            if (id != null)
                output.m_402368_("id", id);
            self().m_20240_(output);
    ...
    default boolean isPushedByFluid(FluidType type) {
        return self().m_6063_() && type.canPushEntity(self());
    }
