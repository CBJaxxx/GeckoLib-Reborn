...
public abstract class GlobalLootModifierProvider implements DataProvider {
    ...
    private CompletableFuture<?> run(CachedOutput cache, HolderLookup.Provider registries) {
        ...
        Path forgePath = this.output.m_247566_(PackOutput.Target.DATA_PACK).resolve("forge/loot_modifiers/global_loot_modifiers.json");
        ...
        toSerialize.forEach(LamdbaExceptionUtils.rethrowBiConsumer((name, instance) -> {
            ...
            entries.add(ResourceLocation.m_339182_(modid, name));
            Path modifierPath = modifierFolderPath.resolve(name + ".json");
            futuresBuilder.add(DataProvider.m_253162_(cache, json, modifierPath));
        }));
        ...
        futuresBuilder.add(DataProvider.m_253162_(cache, forgeJson, forgePath));
