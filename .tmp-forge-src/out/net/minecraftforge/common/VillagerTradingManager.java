...
public class VillagerTradingManager {
    ...
    private static final Lazy<List<Pair<ItemListing[], Integer>>> WANDERER_TRADES = Lazy.of(() -> new ArrayList<Pair<ItemListing[], Integer>>(VillagerTrades.f_35628_));
    ...
    private static Map<ResourceKey<VillagerProfession>, Int2ObjectMap<ItemListing[]>> copyVanillaTrades() {
        ...
        VillagerTrades.f_35627_.forEach((key, value) -> {
    ...
    private static void postWandererEvent() {
        ...
        for (var pool : event.getPools())
            VillagerTrades.f_35628_.add(Pair.of(pool.getEntries().toArray(ItemListing[]::new), pool.getRolls()));
    }
    ...
    private static void postVillagerEvents() {
        ...
        for (VillagerProfession value : ForgeRegistries.VILLAGER_PROFESSIONS) {
            ...
            for (int i = 1; i < 6; i++)
                mutableTrades.put(i, NonNullList.m_122779_());
