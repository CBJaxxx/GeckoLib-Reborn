...
public final class ForgeBiomeModifiers {
    ...
    public static record AddFeaturesBiomeModifier(HolderSet<Biome> biomes, HolderSet<PlacedFeature> features, Decoration step) implements BiomeModifier {
        public static final MapCodec<AddFeaturesBiomeModifier> CODEC = RecordCodecBuilder.mapCodec(builder -> builder.group(
            Biome.f_47432_.fieldOf("biomes").forGetter(AddFeaturesBiomeModifier::biomes),
            PlacedFeature.f_191774_.fieldOf("features").forGetter(AddFeaturesBiomeModifier::features),
            Decoration.f_224188_.fieldOf("step").forGetter(AddFeaturesBiomeModifier::step)
        ).apply(builder, AddFeaturesBiomeModifier::new));

        @Override
        public void modify(Holder<Biome> biome, Phase phase, Builder builder) {
            if (phase == Phase.ADD && this.biomes.m_203333_(biome)) {
    ...
    public static record RemoveFeaturesBiomeModifier(HolderSet<Biome> biomes, HolderSet<PlacedFeature> features, Set<Decoration> steps) implements BiomeModifier {
        public static final MapCodec<RemoveFeaturesBiomeModifier> CODEC = RecordCodecBuilder.mapCodec(builder ->
            builder.group(
                Biome.f_47432_.fieldOf("biomes").forGetter(RemoveFeaturesBiomeModifier::biomes),
                PlacedFeature.f_191774_.fieldOf("features").forGetter(RemoveFeaturesBiomeModifier::features),
                Codec.either(Decoration.f_224188_.listOf(), Decoration.f_224188_).<Set<Decoration>>xmap(
        ...
        @Override
        public void modify(Holder<Biome> biome, Phase phase, Builder builder) {
            if (phase == Phase.REMOVE && this.biomes.m_203333_(biome)) {
                var generationSettings = builder.getGenerationSettings();
    ...
    public record AddSpawnsBiomeModifier(HolderSet<Biome> biomes, WeightedList<SpawnerData> spawners) implements BiomeModifier {
        public static final MapCodec<AddSpawnsBiomeModifier> CODEC = RecordCodecBuilder.mapCodec(builder ->
            builder.group(
                Biome.f_47432_.fieldOf("biomes").forGetter(AddSpawnsBiomeModifier::biomes),
                // Allow either a list or single spawner, attempting to decode the list format first.
                WeightedList.m_387779_(SpawnerData.f_48403_).fieldOf("spawners").forGetter(AddSpawnsBiomeModifier::spawners)
        ...
        public static AddSpawnsBiomeModifier singleSpawn(HolderSet<Biome> biomes, int weight, SpawnerData spawner) {
            return new AddSpawnsBiomeModifier(biomes, WeightedList.m_384569_(List.of(new Weighted<>(spawner, weight))));
        }
        ...
        public void modify(Holder<Biome> biome, Phase phase, Builder builder) {
            if (phase == Phase.ADD && this.biomes.m_203333_(biome)) {
                ...
                for (var weighted : this.spawners.m_387721_()) {
                    var spawner = weighted.f_380007_();
                    spawns.m_48376_(spawner.f_48404_().m_20674_(), weighted.f_380172_(), spawner);
                }
            }
        }
    ...
    public record RemoveSpawnsBiomeModifier(HolderSet<Biome> biomes, HolderSet<EntityType<?>> entityTypes) implements BiomeModifier {
        public static final MapCodec<RemoveSpawnsBiomeModifier> CODEC = RecordCodecBuilder.mapCodec(builder ->
            builder.group(
                Biome.f_47432_.fieldOf("biomes").forGetter(RemoveSpawnsBiomeModifier::biomes),
                RegistryCodecs.m_206277_(ForgeRegistries.Keys.ENTITY_TYPES).fieldOf("entity_types").forGetter(RemoveSpawnsBiomeModifier::entityTypes)
        ...
        public void modify(Holder<Biome> biome, Phase phase, Builder builder) {
            if (phase == Phase.REMOVE && this.biomes.m_203333_(biome)) {
                ...
                for (var category : MobCategory.values())
                    spawns.getSpawner(category).removeIf(data -> this.entityTypes.m_203333_(ForgeRegistries.ENTITY_TYPES.getHolder(data.f_48404_()).orElseThrow()));
            }
        }
