...
public class QuadBakingVertexConsumer implements VertexConsumer {
    private final Map<VertexFormatElement, Integer> ELEMENT_OFFSETS = Util.m_137469_(new IdentityHashMap<>(), map -> {
        int i = 0;
        for (var element : DefaultVertexFormat.f_85811_.getElements())
            map.put(element, DefaultVertexFormat.f_85811_.getOffset(i++) / 4); // Int offset
    });
