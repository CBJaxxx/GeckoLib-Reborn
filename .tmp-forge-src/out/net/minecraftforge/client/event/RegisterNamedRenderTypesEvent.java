...
public final class RegisterNamedRenderTypesEvent implements IModBusEvent {
    ...
    public void register(String name, ChunkSectionLayer blockRenderType, RenderType entityRenderType, RenderType fabulousEntityRenderType) {
        ...
        var key = ResourceLocation.m_339182_(ModLoadingContext.get().getActiveNamespace(), name);
        Preconditions.checkArgument(!renderTypes.containsKey(key), "Render type already registered: " + key);
        Preconditions.checkArgument(entityRenderType.m_110508_() == DefaultVertexFormat.f_85812_, "The entity render type must use the NEW_ENTITY vertex format.");
        Preconditions.checkArgument(fabulousEntityRenderType.m_110508_() == DefaultVertexFormat.f_85812_, "The fabulous entity render type must use the NEW_ENTITY vertex format.");
