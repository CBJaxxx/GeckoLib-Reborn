...
public class ClientCommandHandler {
    ...
    private static void handleClientPlayerLogin(ClientPlayerNetworkEvent.LoggingIn event) {
        final ClientPacketListener connection = event.getPlayer().f_108617_;
        ...
        connection.f_104899_ = mergeServerCommands(new CommandDispatcher<>(), CommandBuildContext.m_255418_(connection.m_105152_(), connection.m_247016_()));
    }
        ...
        return new ClientCommandSourceStack(
            new CommandSource() {
                ...
                public void m_213846_(Component message) {
                    mc.f_91065_.m_93076_().m_93785_(message);
                }
    ...
    public static boolean runCommand(String command) {
        ...
        //    Minecraft.getInstance().player.sendSystemMessage(Component.literal("").append(execution.getComponent()).withStyle(ChatFormatting.RED));
            ...
            mc.f_91065_.m_93076_().m_93785_(
                Component.m_237113_("").m_7220_(ComponentUtils.m_130729_(syntax.getRawMessage())).m_130940_(ChatFormatting.RED)
            );
            if (syntax.getInput() != null && syntax.getCursor() >= 0) {
                int position = Math.min(syntax.getInput().length(), syntax.getCursor());
                MutableComponent details = Component.m_237113_("")
                        .m_130940_(ChatFormatting.GRAY)
                        .m_130938_((style) -> style
                                .m_131142_(new ClickEvent.SuggestCommand(reader.getString())));
                if (position > 10)
                    details.m_130946_("...");

                details.m_130946_(syntax.getInput().substring(Math.max(0, position - 10), position));
                if (position < syntax.getInput().length())
                    details.m_7220_(Component.m_237113_(syntax.getInput().substring(position)).m_130944_(ChatFormatting.RED, ChatFormatting.UNDERLINE));

                details.m_7220_(Component.m_237115_("command.context.here").m_130944_(ChatFormatting.RED, ChatFormatting.ITALIC));
                mc.f_91065_.m_93076_().m_93785_(Component.m_237113_("").m_7220_(details).m_130940_(ChatFormatting.RED));
            }
        } catch (Exception generic) { // Probably thrown by the command{
            MutableComponent message = Component.m_237113_(generic.getMessage() == null ? generic.getClass().getName() : generic.getMessage());
            mc.f_91065_.m_93076_().m_93785_(
                Component.m_237115_("command.failed")
                    .m_130940_(ChatFormatting.RED)
                    .m_130938_(style -> style.m_131144_(new HoverEvent.ShowText(message)))
            );
