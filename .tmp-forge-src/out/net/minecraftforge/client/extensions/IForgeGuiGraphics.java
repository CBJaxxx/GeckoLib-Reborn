...
public interface IForgeGuiGraphics {
    ...
    default void blitWithBorder(ResourceLocation texture, int x, int y, int u, int v, int width, int height, int textureWidth, int textureHeight, int topBorder, int bottomBorder, int leftBorder, int rightBorder) {
        ...
        // Draw Border
        // Top Left
        self().m_403448_(RenderPipelines.f_379833_, texture, x,                            y,                            0, u,                            v,                            leftBorder,  topBorder, 256, 256);
        // Top Right
        self().m_403448_(RenderPipelines.f_379833_, texture, x + leftBorder + canvasWidth, y,                            0, u + leftBorder + fillerWidth, v,                            rightBorder, topBorder, 256, 256);
        // Bottom Left
        self().m_403448_(RenderPipelines.f_379833_, texture, x,                            y + topBorder + canvasHeight, 0, u,                            v + topBorder + fillerHeight, leftBorder,  bottomBorder, 256, 256);
        // Bottom Right
        self().m_403448_(RenderPipelines.f_379833_, texture, x + leftBorder + canvasWidth, y + topBorder + canvasHeight, 0, u + leftBorder + fillerWidth, v + topBorder + fillerHeight, rightBorder, bottomBorder, 256, 256);

        for (int i = 0; i < xPasses + (remainderWidth > 0 ? 1 : 0); i++) {
            // Top Border
            self().m_403448_(RenderPipelines.f_379833_, texture, x + leftBorder + (i * fillerWidth), y,                            0, u + leftBorder, v,                            (i == xPasses ? remainderWidth : fillerWidth), topBorder,    256, 256);
            // Bottom Border
            self().m_403448_(RenderPipelines.f_379833_, texture, x + leftBorder + (i * fillerWidth), y + topBorder + canvasHeight, 0, u + leftBorder, v + topBorder + fillerHeight, (i == xPasses ? remainderWidth : fillerWidth), bottomBorder, 256, 256);

            // Throw in some filler for good measure
            for (int j = 0; j < yPasses + (remainderHeight > 0 ? 1 : 0); j++)
                self().m_403448_(RenderPipelines.f_379833_, texture, x + leftBorder + (i * fillerWidth), y + topBorder + (j * fillerHeight), 0, u + leftBorder, v + topBorder, (i == xPasses ? remainderWidth : fillerWidth), (j == yPasses ? remainderHeight : fillerHeight), 256, 256);
        }

        // Side Borders
        for (int j = 0; j < yPasses + (remainderHeight > 0 ? 1 : 0); j++) {
            // Left Border
            self().m_403448_(RenderPipelines.f_379833_, texture, x,                            y + topBorder + (j * fillerHeight), 0, u,                            v + topBorder, leftBorder,  (j == yPasses ? remainderHeight : fillerHeight), 256, 256);
            // Right Border
            self().m_403448_(RenderPipelines.f_379833_, texture, x + leftBorder + canvasWidth, y + topBorder + (j * fillerHeight), 0, u + leftBorder + fillerWidth, v + topBorder, rightBorder, (j == yPasses ? remainderHeight : fillerHeight), 256, 256);
        }
    }
    ...
    default void blitInscribed(ResourceLocation texture, int x, int y, int boundsWidth, int boundsHeight, int rectWidth, int rectHeight, boolean centerX, boolean centerY) {
        ...
        self().m_280218_(RenderPipelines.f_379833_, texture, x, y, 0.0f, 0.0f, boundsWidth, boundsHeight, boundsWidth, boundsHeight);
    }
}
