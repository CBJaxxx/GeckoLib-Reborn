...
public class LanguageHook {
    ...
    private static void loadLocaleData(final InputStream inputstream) {
        try {
            ...
            JsonObject jsonobject = GsonHelper.m_13918_(jsonelement, "strings");

            for (var entry : jsonobject.entrySet()) {
                String s = PATTERN.matcher(GsonHelper.m_13805_(entry.getValue(), entry.getKey())).replaceAll("%$1s");
    ...
    private static void loadLanguage(String langName, MinecraftServer server) {
        ...
        var resourceManager = server.getServerResources().f_206584_();
        var byNamespace = new HashMap<String, List<PackResources>>();

        var packs = new ArrayList<>(resourceManager.m_7536_().toList());
        ...
        while (!packs.isEmpty()) {
            ...
            for (var namespace : pack.m_5698_(PackType.CLIENT_RESOURCES)) {
        ...
        for (var entry : byNamespace.entrySet()) {
            ...
            var path = ResourceLocation.m_339182_(entry.getKey(), langFile);
            for (var pack : entry.getValue()) {
                var io = pack.m_214146_(PackType.CLIENT_RESOURCES, path);
                if (io != null)
                    languages.add(io);
            }

            while (!languages.isEmpty()) {
                try (var stream = languages.removeLast().m_247737_()) {
