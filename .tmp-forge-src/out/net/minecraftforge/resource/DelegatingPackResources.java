...
public class DelegatingPackResources extends AbstractPackResources {
    ...
    @Nullable
    @Override
    public <T> T m_5550_(MetadataSectionType<T> deserializer) throws IOException {
        return deserializer.f_371245_().equals("pack") ? (T) this.packMeta : null;
    }

    @Override
    public void m_8031_(PackType type, String resourceNamespace, String paths, ResourceOutput resourceOutput) {
    ...
    public IoSupplier<InputStream> m_214146_(PackType type, ResourceLocation location) {
        for (PackResources pack : getCandidatePacks(type, location)) {
            IoSupplier<InputStream> ioSupplier = pack.m_214146_(type, location);
            if (ioSupplier != null)
                return ioSupplier;
        }
    ...
    private class Supplier implements Pack.ResourcesSupplier {
        @Override
        public PackResources m_293078_(PackLocationInfo p_332103_) {
            return DelegatingPackResources.this;
        }

        @Override
        public PackResources m_247679_(PackLocationInfo p_330351_, Metadata p_333429_) {
            return DelegatingPackResources.this;
        }
    }
}
