...
public class ResourcePackLoader {
    ...
    private static void findPacks(Consumer<Pack> packAcceptor, boolean client) {
        ...
        var version = SharedConstants.m_183709_().m_403307_(type);
        ...
        for (var mod : ModList.get().getModFiles()) {
            ...
            var info = new PackLocationInfo(name, Component.m_237113_(file.getFileName()), PackSource.f_10527_, Optional.empty());
            var meta = Pack.m_324832_(info, supplier, version);
            Pack pack = null;
            if (meta != null)
        ...
        if (!hiddenPacks.isEmpty()) {
            var info = new PackLocationInfo("mod_resources", Component.m_237110_("fml.resources.modresources", hiddenPacks.size()), PackSource.f_10527_, Optional.empty());
            @SuppressWarnings("resource")
            var delegating = new DelegatingPackResources(info,
                new PackMetadataSection(
                    Component.m_237110_("fml.resources.modresources", hiddenPacks.size()),
                    version,
                    Optional.empty()
                ),
                hiddenPacks
            ...
            var wrapper = new PackLocationInfo("mod_resources", Component.m_237113_("Mod Resources"), PackSource.f_10527_, Optional.empty());
            var wrapperCfg = new PackSelectionConfig(true, Position.BOTTOM, false);
            var modResourcesPack = Pack.m_245429_(wrapper, supplier, type, wrapperCfg);
