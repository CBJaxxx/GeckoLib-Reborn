...
public class FluidBucketWrapper implements IFluidHandlerItem, ICapabilityProvider {
    ...
    public FluidStack getFluid() {
        ...
        else if (container.m_150930_(Items.f_42455_) && ForgeMod.MILK.isPresent())
    ...
    protected void setFluid(@NotNull FluidStack fluidStack) {
        if (fluidStack.isEmpty())
            container = new ItemStack(Items.f_42446_);
        else
    ...
    public int fill(FluidStack resource, FluidAction action) {
        if (container.m_41613_() != 1 || resource.getAmount() < FluidType.BUCKET_VOLUME || container.m_150930_(Items.f_42455_) || !getFluid().isEmpty() || !canFillFluidType(resource))
            return 0;
    ...
    public FluidStack drain(FluidStack resource, FluidAction action) {
        if (container.m_41613_() != 1 || resource.getAmount() < FluidType.BUCKET_VOLUME)
            return FluidStack.EMPTY;
    ...
    public FluidStack drain(int maxDrain, FluidAction action) {
        if (container.m_41613_() != 1 || maxDrain < FluidType.BUCKET_VOLUME)
            return FluidStack.EMPTY;
