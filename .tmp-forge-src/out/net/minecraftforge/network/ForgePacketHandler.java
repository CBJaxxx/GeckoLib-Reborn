...
public class ForgePacketHandler {
    ...
    void handleClientAck(Acknowledge msg, CustomPayloadEvent.Context ctx) {
        ...
        if (consumer != null) {
            ...
            ctx.getConnection().m_129507_(Component.m_237113_("Illegal Acknowledge packet received, unknown token: " + Integer.toHexString(msg.token())));
        }
    }
    ...
    void handleChannelVersions(ChannelVersions list, CustomPayloadEvent.Context ctx) {
        ...
        if (invalid != null) {
            ...
            ctx.getConnection().m_129507_(Component.m_237113_("Connection closed - mismatched mod channel list"));
            return;
        }
    ...
    void handleRegistryList(RegistryList list, CustomPayloadEvent.Context ctx) {
        ...
        for (var key : list.datapacks()) {
            if (!clientDataPackRegistries.contains(key)) {
                LOGGER.error(MARKER, "Missing required datapack registry: {}", key.m_135782_());
                missing.add(key.m_135782_().toString());
            }
        }

        if (!missing.isEmpty()) {
            ctx.getConnection().m_129507_(Component.m_237110_("fml.menu.multiplayer.missingdatapackregistries", String.join(", ", missing)));
            return;
        }
    ...
    void handleRegistryData(RegistryData msg, CustomPayloadEvent.Context ctx) {
        ...
        if (!this.registriesToReceive.remove(msg.name())) {
            ...
            ctx.getConnection().m_129507_(Component.m_237113_("Illegal Registry Data packet received, unexpected registry: " + msg.name()));
            return;
        }
    ...
    private boolean handleRegistryLoading(CustomPayloadEvent.Context ctx) {
        ...
        ctx.enqueueWork(() -> {
            ...
            if (!missingData.isEmpty()) {
                ...
                var missingMods = missingData.values().stream()
                    .map(ResourceLocation::m_135827_)
                    .distinct()
                    .toList();
                ...
                for (var id : missingMods) {
                    var key = ResourceLocation.m_339182_(id, "");
        ...
        if (mismatchData != null) {
            ...
            ctx.getConnection().m_129507_(Component.m_237113_("Failed to synchronize registry data from server, closing connection"));
            return false;
        }
